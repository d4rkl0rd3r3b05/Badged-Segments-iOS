         ...
         withShining:(BOOL)shining
{
    ...
    if (self != nil) {
        ...
        self.badgeInsetColor = [UIColor colorWithRed:209 / 255.0
                                               green:72 / 255.0
                                                blue:53 / 255.0
                                               alpha:1];
        self.badgeCornerRoundness = 0.1;
            ...
            withShining:(BOOL)shining
{
    ...
    if (self != nil) {
        ...
        self.badgeCornerRoundness = 0.10;
        self.badgeScaleFactor = scale;
        //		self.badgeShining = shining;
        [self autoBadgeSizeWithString:badgeString];
    }
...
// Use this method if you want to change the badge text after the first
// rendering
- (void)autoBadgeSizeWithString:(NSString*)badgeString
{
    ...
    CGSize stringSize =
        [badgeString sizeWithFont:[UIFont boldSystemFontOfSize:12]];
    CGFloat flexSpace;
...
+ (CustomBadge*)customBadgeWithString:(NSString*)badgeString
{
    return [[[self alloc] initWithString:badgeString
                               withScale:1.0
                             withShining:YES] autorelease];
}

// Creates a Badge with a given Text, Text Color, Inset Color, Frame (YES/NO)
// and Frame Color
+ (CustomBadge*)customBadgeWithString:(NSString*)badgeString
                      withStringColor:(UIColor*)stringColor
                       withInsetColor:(UIColor*)insetColor
...
- (void)drawRoundedRectWithContext:(CGContextRef)context withRect:(CGRect)rect
{
    ...
    CGContextAddArc(
        context, maxX - radius, minY + radius, radius, M_PI + (M_PI / 2), 0, 0);
    CGContextAddArc(
        context, maxX - radius, maxY - radius, radius, 0, M_PI / 2, 0);
    CGContextAddArc(
        context, minX + radius, maxY - radius, radius, M_PI / 2, M_PI, 0);
    CGContextAddArc(
        context, minX + radius, minY + radius, radius, M_PI, M_PI + M_PI / 2, 0);
    //	CGContextSetShadowWithColor(context, CGSizeMake(1.0,1.0), 3, [[UIColor
    //blackColor] CGColor]);
...
- (void)drawShineWithContext:(CGContextRef)context withRect:(CGRect)rect
{
    ...
    gradient = CGGradientCreateWithColorComponents(
        cspace, components, locations, num_locations);
...
- (void)drawRect:(CGRect)rect
{
    ...
    [self drawRoundedRectWithContext:context
                            withRect:rect];

    if (self.badgeShining) {
        [self drawShineWithContext:context
                          withRect:rect];
    }
    ...
    if ([self.badgeText length] > 0) {
        ...
        [self.badgeText
            drawAtPoint:CGPointMake((rect.size.width / 2 - textSize.width / 2),
...
- (void)changeBadgeType:(SEGMENT_BADGE_TYPE)argBadgeType
{
    ...
    case NORMAL_SELECTED_BADGES:
        lineSize = 1;
        self.badgeInsetColor = [UIColor clearColor];
        self.badgeTextColor = [UIColor whiteColor];
        self.badgeFrameColor = [UIColor whiteColor];
        break;

    case NORMAL_UNSELECTED_BADGES:
        lineSize = 1;
        self.badgeInsetColor = [UIColor clearColor];
        self.badgeTextColor = BLUE_COLOR; //[UIColor colorWithRed:63/255.0
        //green:134/255.0 blue:202/255.0
        //alpha:1.0];
        self.badgeFrameColor = BLUE_COLOR; //[UIColor colorWithRed:63/255.0
        //green:134/255.0 blue:202/255.0
        //alpha:1.0];
        break;
